...
export async function buildGrowthData(){
  ...
  const monthsArr = Array.from(monthSet).sort((a,b)=>b-a);

  const m0 = monthsArr[0];
  const m1 = monthsArr[1];
  const m2 = monthsArr[2];

  const m0y = Math.floor(m0/100), m0m = m0%100;
  const m1y = Math.floor(m1/100), m1m = m1%100;
  const m2y = Math.floor(m2/100), m2m = m2%100;
