...
export function initSORTab() {
  window.renderSORTab = async function () {
    ...
    root.innerHTML = `
      <section class="panel">
        ...
        <div style="padding:16px;display:grid;gap:12px;grid-template-columns:repeat(4,minmax(0,1fr));align-items:end;">
          ...
          <div><label style="font-size:12px;color:#666;">Sort</label><select id="sorSort"><option value="sales">Sales</option><option value="projection">Projection</option><option value="ship">Shipment</option><option value="recall">Recall</option></select></div>
        </div>
        ...
        ${rows.length > LIMIT ? `<button id="sorMore" class="load-more">Load More</button>` : ""}
    ...
    document.getElementById("salesDays").onchange = e => { SALES_DAYS = Number(e.target.value); window.renderSORTab(); };
    document.getElementById("coverDays").onchange = e => { COVER_DAYS = Number(e.target.value); window.renderSORTab(); };
    document.getElementById("recallDays").onchange = e => { RECALL_DAYS = Number(e.target.value); window.renderSORTab(); };
    document.getElementById("sorSort").onchange = e => { SORT = e.target.value; window.renderSORTab(); };
